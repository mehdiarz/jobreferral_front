...
import type {
  GetAllPropertyAppraisalsParams,
  PropertyAppraisalOutputDto,
} from "./types";

export async function getAllPropertyAppraisals(
  params?: GetAllPropertyAppraisalsParams,
): Promise<{ items: PropertyAppraisalOutputDto[]; totalCount: number }> {
  ...
  const url = query
    ? `/services/app/PropertyAppraisalCrud/GetAll?${query}`
    : "/services/app/PropertyAppraisalCrud/GetAll";
