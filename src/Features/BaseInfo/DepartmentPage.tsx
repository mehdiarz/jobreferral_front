...
export default function DepartmentPage() {
    ...
    const departmentsQuery = useQuery({
        queryKey: ["departments", filters, pagination.pageIndex, pagination.pageSize],
        queryFn: () => getAllDepartments(),
        select: (data): DepartmentsQueryData => {
            const apiData = data as DepartmentsApiResponse;
            const allItems: DepartmentItem[] =
                apiData?.items ?? apiData?.result?.items ?? apiData?.listResult ?? apiData?.data ?? [];

            const titleFilter = filters.find((f) => f.key === "title")?.value?.trim().toLocaleLowerCase("fa") ?? "";
            const codeFilter = filters.find((f) => f.key === "code")?.value?.trim().toLocaleLowerCase("fa") ?? "";

            const filteredItems = allItems.filter((item) => {
                const itemTitle = String(item.title ?? "").trim().toLocaleLowerCase("fa");
                const itemCode = String(item.code ?? "").trim().toLocaleLowerCase("fa");
                return (!titleFilter || itemTitle.includes(titleFilter)) && (!codeFilter || itemCode.includes(codeFilter));
            });
            ...
            return { listResult: filteredItems.slice(startIndex, startIndex + pagination.pageSize), total, totalPages };
        },
    });
    ...
    const parentDepartmentOptions = useMemo(
        () => [
            { id: "", title: "بدون والد" },
            ...(departmentsQuery.data?.listResult ?? []).map((item: DepartmentItem) => ({
                id: String(item.id),
                title: item.title ?? "",
            })),
        ],
    ...
    const createMutation = useMutation({
        ...
        onError: (error) => {
            showToast(error instanceof Error ? error.message : "خطا در ثبت اطلاعات", "error", 5000);
        },
    });
    ...
    const updateMutation = useMutation({
        ...
        onError: (error) => {
            showToast(error instanceof Error ? error.message : "خطا در ویرایش اطلاعات", "error", 5000);
        },
    });
    ...
    const deleteMutation = useMutation({
        ...
        onError: (error) => {
            showToast(error instanceof Error ? error.message : "خطا در حذف", "error", 5000);
        },
    });
    ...
    const handleSubmitForm = () => {
        ...
        if (!code) { showToast("وارد کردن کد الزامی است", "error"); return; }
        if (!title) { showToast("وارد کردن عنوان الزامی است", "error"); return; }

        const body = {
            code,
            title,
            departmentGradeId: formData.departmentGradeId ? Number(formData.departmentGradeId) : undefined,
            parentId: formData.parentId ? Number(formData.parentId) : 0,
            regionId: formData.regionId ? Number(formData.regionId) : undefined,
            documentTypeId: formData.documentTypeId ? Number(formData.documentTypeId) : undefined,
    ...
    const handleExportExcel = () => {
        ...
        const headers = ["عنوان", "کد", "رتبه دپارتمان", "دپارتمان والد", "منطقه استانی", "نوع مدارک پیوست", "توضیحات", "وضعیت"];

        const csvRows = rows.map((item) => {
            const grade = (departmentGradesQuery.data ?? []).find((g: DepartmentGradeItem) => g.id === item.departmentGradeId);
            const parent = (departmentsQuery.data?.listResult ?? []).find((d: DepartmentItem) => d.id === item.parentId);
            const region = (regionsQuery.data ?? []).find((r: RegionItem) => r.id === item.regionId);
            const doc = (documentTypesQuery.data ?? []).find((d: DocumentTypeItem) => d.id === item.documentTypeId);
            return [
                item.title ?? "", item.code ?? "", grade?.title ?? "", parent?.title ?? "",
                region?.title ?? "", doc?.title ?? "", item.description ?? "", item.isActive ? "فعال" : "غیرفعال",
            ];
        });
        ...
        link.href = url; link.download = "departments.csv"; link.click();
    ...
    return (
        <MainLayout.Main maxWidth="screen-xl">
            ...
            <Modal
                ...
                renderContent={() => (
                    <FluidGrid className="gap-4">
                        <FluidCol colSpan={12}>
                            <FormInput id="modal-title" name="modal-title" label="عنوان" value={formData.title}
                                       onChange={(v) => setFormData((p) => ({ ...p, title: v }))} dir="rtl" required />
                        </FluidCol>
                        <FluidCol colSpan={12}>
                            <FormInput id="modal-code" name="modal-code" label="کد" value={formData.code}
                                       onChange={(v) => setFormData((p) => ({ ...p, code: v }))} dir="ltr" required />
                        </FluidCol>
                        <FluidCol colSpan={12}>
                            <FormSelect id="modal-departmentGradeId" name="modal-departmentGradeId" label="رتبه دپارتمان"
                                        value={formData.departmentGradeId}
                                        onChange={(v) => setFormData((p) => ({ ...p, departmentGradeId: v }))}
                                        options={departmentGradeOptions} />
                        </FluidCol>
                        <FluidCol colSpan={12}>
                            <FormSelect id="modal-parentId" name="modal-parentId" label="دپارتمان والد"
                                        value={formData.parentId}
                                        onChange={(v) => setFormData((p) => ({ ...p, parentId: v }))}
                                        options={parentDepartmentOptions} />
                        </FluidCol>
                        <FluidCol colSpan={12}>
                            <FormSelect id="modal-regionId" name="modal-regionId" label="منطقه استانی"
                                        value={formData.regionId}
                                        onChange={(v) => setFormData((p) => ({ ...p, regionId: v }))}
                                        options={regionOptions} />
                        </FluidCol>
                        <FluidCol colSpan={12}>
                            <FormSelect id="modal-documentTypeId" name="modal-documentTypeId" label="نوع مدارک پیوست"
                                        value={formData.documentTypeId}
                                        onChange={(v) => setFormData((p) => ({ ...p, documentTypeId: v }))}
                                        options={documentTypeOptions} />
                        </FluidCol>
                        <FluidCol colSpan={12}>
                            <FormTextarea id="modal-description" name="modal-description" label="توضیحات"
                                          value={formData.description}
                                          onChange={(v) => setFormData((p) => ({ ...p, description: v }))} rows={3} dir="rtl" />
                        </FluidCol>
                        <FluidCol colSpan={12}>
                            <FormSelect id="modal-isActive" name="modal-isActive" label="وضعیت"
            ...
            <Modal
                ...
                footerButtons={
                    <div className="flex gap-2">
                        <FormButton title="حذف" variant="danger" isLoading={deleteMutation.isPending}
                                    onClick={() => { if (itemToDelete) deleteMutation.mutate(itemToDelete.id); }} />
                        <FormButton title="انصراف" variant="secondary" disabled={deleteMutation.isPending}
                                    onClick={() => setItemToDelete(null)} />
                    </div>
                }
                renderContent={() => (
                    <p>آیا از حذف <strong>{itemToDelete ? itemToDelete.title ?? "" : ""}</strong> اطمینان دارید؟</p>
                )}
            />
        </MainLayout.Main>
    );
