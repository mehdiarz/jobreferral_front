...
type RequestTypeForm = { code: string; title: string; description: string };
type TableFilter = { key: string; value: string };
type RequestTypesApiResponse = {
  items?: RequestTypeItem[];
  result?: { items?: RequestTypeItem[] };
...
const emptyForm: RequestTypeForm = { code: "", title: "", description: "" };
...
export default function RequestTypesPage() {
  ...
  const [formData, setFormData] = useState<RequestTypeForm>(emptyForm);
  const [editingId, setEditingId] = useState<number | null>(null);
  const [itemToDelete, setItemToDelete] = useState<RequestTypeItem | null>(
    null,
  );
  ...
  const createMutation = useMutation({
    mutationFn: (body: CreateRequestTypeBody) => createRequestType(body),
    onSuccess: () => {
      showToast("نوع درخواست با موفقیت ثبت شد", "success");
      closeFormModal();
  ...
  const updateMutation = useMutation({
    mutationFn: (body: EditRequestTypeBody) => editRequestType(body),
    onSuccess: () => {
  ...
  const handleOpenEditModal = useCallback((item: RequestTypeItem) => {
    setFormMode("edit");
  ...
  const handleDeleteClick = useCallback(
    (item: RequestTypeItem) => setItemToDelete(item),
    [],
  );
  ...
  const handleExportPdf = () => {
    ...
    printWindow.document.write(
      `<html dir="rtl" lang="fa"><head><title>PDF</title><style>body{font-family:Tahoma,Arial;direction:rtl;padding:24px}table{width:100%;border-collapse:collapse}th,td{border:1px solid #ddd;padding:8px;text-align:right}th{background:#f3f4f6}</style></head><body><h2>لیست انواع درخواست</h2><table><thead><tr><th>کد</th><th>عنوان</th></tr></thead><tbody>${tableRows}</tbody></table><script>window.onload=function(){window.print()}</script></body></html>`,
    );
  ...
  return (
    <MainLayout.Main maxWidth="screen-xl">
      ...
      <Modal
        ...
        isRTL
        header={
          formMode === "create" ? "افزودن نوع درخواست" : "ویرایش نوع درخواست"
        }
