...
export function DepartmentEngineeringManagementReferralReviewPage({
  departmentType,
}: EngineeringManagementReferralReviewPageProps) {
  ...
  // فرم ثبت‌شده توسط ستاد؛ در این صفحه قابل ویرایش است.
  const [mainOfficeAppraisal, setMainOfficeAppraisal] =
    useState<PropertyAppraisalOutputDto | null>(null);

  // فرم ثبت‌شده توسط شعبه / منطقه / سایر واحدها؛ فقط قابل مشاهده است.
  ...
  const requestsQuery = useQuery({
    queryKey: [
      "requests-engineering-management-referral-review",
      departmentType.id,
    ...
    select: (data) => {
      const items = ((data?.items ?? []) as RequestItem[]).filter(
        (r) =>
          r.requestStatusCode ===
            REQUEST_STATUS_CODES.engineeringManagementReferral ||
          r.requestStatusCode ===
            REQUEST_STATUS_CODES.engineeringManagementReturned,
      );
      return {
        listResult: items,
  ...
  const handleView = useCallback(
    async (req: RequestItem) => {
      ...
      // جلوگیری از باقی‌ماندن داده‌های درخواست قبلی
      ...
      try {
        ...
        try {
          ...
          // فرم واحدهای غیرستادی: فقط خواندنی
          const externalForm =
            appraisals.find(
              (appraisal) =>
                Number(appraisal.creatorDepartmentId) !==
          ...
          // فقط اطلاعات فرم ستاد وارد مودال قابل ویرایش شود.
  ...
  const handleSaveAppraisal = useCallback(async () => {
    ...
    // فقط فرم ستاد اجازه ویرایش دارد.
    if (
    ...
    try {
      ...
      /*
       * null، undefined و string خالی ارسال نشوند.
       * false حذف نمی‌شود تا مقدار checkboxها حفظ شود.
       */
  ...
  return (
    <MainLayout.Main maxWidth="screen-xl">
      <PageTitle title={`بررسی و ارجاع توسط ${departmentType.name}`} />
      ...
      <Modal
        isOpen={isDetailOpen}
        isRTL
        header="بررسی و ارجاع توسط مدیریت مهندسی و پشتیبانی"
...
export default function EngineeringManagementReferralReviewPage() {
  return (
    <DepartmentEngineeringManagementReferralReviewPage
