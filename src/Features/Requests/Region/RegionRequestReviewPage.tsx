...
// ─── Main Component ──────────────────────────────────────────────
interface RegionRequestReviewPageProps {
...
export function DepartmentRegionRequestReviewPage({
  departmentType,
}: RegionRequestReviewPageProps) {
  ...
  const requestsQuery = useQuery({
    ...
    select: (data) => {
      const items = ((data?.items ?? []) as RequestItem[]).filter(
        (r) => r.requestStatusCode === REQUEST_STATUS_CODES.regionReferral,
      );
      return {
        listResult: items,
  ...
  return (
    <MainLayout.Main maxWidth="screen-xl">
      <PageTitle title={`کارتابل ${departmentType.name}`} />
      ...
      <Modal
        isOpen={isDetailOpen}
        isRTL
        header={`کارتابل ${departmentType.name}`}
...
export default function RegionRequestReviewPage() {
  return (
    <DepartmentRegionRequestReviewPage
