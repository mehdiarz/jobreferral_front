...
// ─── Main Component ──────────────────────────────────────────────
interface RegionManagerApprovalPageProps {
...
export function DepartmentRegionManagerApprovalPage({
  ...
  const requestsQuery = useQuery({
    queryKey: [
      "requests-region-manager-approval",
      departmentType.id,
    ...
    select: (data) => {
      const items = ((data?.items ?? []) as RequestItem[]).filter(
        (r) => r.requestStatusCode === REQUEST_STATUS_CODES.regionManagerReview,
      );
      return {
        listResult: items,
  ...
  return (
    <MainLayout.Main maxWidth="screen-xl">
      <PageTitle title="بررسی و امضا توسط مدیر منطقه" />
      ...
      <Modal
        isOpen={isDetailOpen}
        isRTL
        header="بررسی و امضا توسط مدیر منطقه"
...
export default function RegionManagerApprovalPage() {
  return (
    <DepartmentRegionManagerApprovalPage
