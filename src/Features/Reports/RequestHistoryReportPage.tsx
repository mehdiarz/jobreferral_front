...
import { getPagedRequestHistories } from "../../services/RequestHistoryReport/getPagedRequestHistories";
import { exportRequestHistoriesToExcel } from "../../services/RequestHistoryReport/exportRequestHistoriesToExcel";
...
import type {
  ...
  RequestHistoryReportDto,
} from "../../services/RequestHistoryReport/types";
...
export default function RequestHistoryReportPage() {
  ...
  const query = useQuery({
    ...
    queryFn: () => {
      const params: GetPagedRequestHistoriesParams = {
      ...
      return getPagedRequestHistories(params);
    },
    select: (data) => ({
  ...
  const exportMutation = useMutation({
    mutationFn: () => {
      ...
      return exportRequestHistoriesToExcel(body);
    },
  ...
  const historyColumns = useMemo<ColumnDef<RequestHistoryReportDto, unknown>[]>(
    () => [
      {
        accessorKey: "requestCode",
        header: "کد درخواست",
        cell: ({ row }) => displayValue(row.original.requestCode),
      },
      {
      ...
      },
      {
        ...
        header: "تاریخ اقدام",
        cell: ({ row }) => {
          const value = row.original.actionDate ?? row.original.creationTime;
          return value ? (
      ...
      },
      {
        ...
        cell: ({ row }) => (
          <button
            type="button"
            className="inline-flex items-center gap-1 rounded-lg cursor-pointer bg-blue-50 px-3 py-1.5 text-xs font-semibold text-blue-700 hover:bg-blue-100"
            onClick={() =>
  ...
  return (
    <MainLayout.Main maxWidth="screen-xl">
      <PageTitle title="گزارش تاریخچه درخواست‌ها" />
      <div className="mb-4 rounded-2xl border border-blue-100 bg-blue-50/50 p-4">
        ...
        <div className="grid grid-cols-1 gap-3 md:grid-cols-2 lg:grid-cols-4">
          ...
          {/* باکس نمایش وضعیت/نام کاربر استعلام‌شده */}
